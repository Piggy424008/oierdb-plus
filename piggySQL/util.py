...
def __main__():
    ...
    with open('./piggySQL/static/contests.json', encoding="utf8") as f:
        for contest in json.load(f):
            Contest.create(contest)

    with open('./piggySQL/static/grades.json', encoding="utf8") as f:
        g_ = json.load(f)
    g_initial = g_['initial']
    g_element = g_['element']
    g_special = g_['special']

    with open('./piggySQL/static/surnames.json', encoding="utf8") as f:
        surnames = json.load(f)

    with open('./piggySQL/static/scoring.json', encoding="utf8") as f:
        scoring = json.load(f)
